...
<?php 
	...
	$query = "SELECT count(company_registration_number) AS total_number_of_rows FROM provider";
	...
	$start = ($page_number-1)*$rows_per_page;
		...
		//link for first page
		$first ="<a href = 'admin-index-provider.php?p=1'>First</a>";

		//link for last page
		$last_page = ceil($total_number_of_rows/$rows_per_page);
		$last ="<a href ='admin-index-provider.php?p={$last_page}'>Last</a>";
		...
		if ($page_number>=$last_page) {
			...
			$next="<a href = 'admin-index-provider.php?p={$next_page}'>Next</a>";
		}
		//link for previous
		if ($page_number==1) {
			$first="<a>First</a>";
			...
			$previous="<a href = 'admin-index-provider.php?p={$previous_page}'>Previous</a>";
		}
...
<?php  
	...
	function delete_button($crn,$page_no,$con){

		$delete_query = "SELECT is_deleted FROM provider WHERE company_registration_number='{$crn}' LIMIT 1";
		...
		if ($delete_query) {
			...
			echo "<form action =\"admin-index-provider.php?p={$page_no}\" method=\"POST\" style='float:left;'>";
			echo "<input type=\"text\" name=\"company_registration_number\" value=\"{$crn}\" hidden>";
	...
	if (isset($_POST["restore"])) {
		...
		$restore_query ="UPDATE provider SET is_deleted = 0 WHERE company_registration_number = '{$_POST['company_registration_number']}' LIMIT 1";

		$result_restore = mysqli_query($connection,$restore_query);//restore provide

		if ($result_restore) {

			$delete_all_ad_query="UPDATE job_ad SET is_delete=0 WHERE company_registration_number='{$_POST['company_registration_number']}'";
	...
	if (isset($_POST["delete"])) {
		...
		$delete_query ="UPDATE provider SET is_deleted = 1 WHERE company_registration_number = '{$_POST['company_registration_number']}' LIMIT 1";

		$result_delete = mysqli_query($connection,$delete_query);//delete provider

		if ($result_delete) {
			$delete_all_ad_query="UPDATE job_ad SET is_delete=1 WHERE company_registration_number='{$_POST['company_registration_number']}'";
...
.section-content .content table tr td:nth-child(3) h3 a{
	color: #0e13af;
...
.section-content .content table tr td:nth-child(5) button{
	margin-left: 32px;
}
	...
	<div class="section-content">
		<div class="content">
				<?php 
					...
					$total_ad_query ="SELECT * FROM provider ORDER BY company_registration_number  LIMIT {$start},{$rows_per_page} ";
					...
					$ads_result = mysqli_query($connection,$total_ad_query);
						...
						if (mysqli_num_rows($ads_result)!=0) {
							 ...
							 echo "<table>";
							 echo "<tr><th>No</th><th>Provider Profile Picture</th><th>Provider Details</th><th>Social Media</th><th>Status</th></tr>";
							 ...
							 $x=($page_number-1)*$rows_per_page;
								 while ($adds = mysqli_fetch_assoc($ads_result)) {
								 	...
								 	echo "<td>";
								 	echo "<h2><i class='fas fa-home'></i>" . $adds["company_name"] . "</h2>";
								 	echo "<h3><i class='fas fa-map-marker-alt'></i>" . $adds["address"] . "</h3><h3><i class='fas fa-phone'></i>" . $adds["company_phone_number"] . "</h3><h3><i class='fas fa-envelope'></i>" . $adds["company_email"] . "</h3><h3><i class='fas fa-globe'></i><a href='http://" . $adds["company_website"] . "' target=\"_block\">{$adds["company_website"]}</a></h3>";
								 	echo "</td>";
								 	echo "<td>
								 			<a href=\"http://www.facebook.com/{$adds["facebook"]}\" target = '_blank'><i class='fab fa-facebook'></i></a>
								 			<a href=\"http://www.twitter.com/{$adds["twitter"]}\" target = '_blank'><i class='fab fa-twitter'></i></a>
								 			<a href=\"http://www.linkedin.com/{$adds["linked_in"]}\" target = '_blank'><i class='fab fa-linkedin'></i></a>
								 		</td>";
								 	echo "<td>";
								 		delete_button($adds["company_registration_number"],$page_number,$connection);
								 	echo "</td>";
								 	echo "</tr>";
								 }
								 echo "</table>";;
