...
<?php 
	...
	if (!isset($_GET["crn"]) && !isset($_GET["ad_no"]) ) {
		header("location:index.php");
	}
	else{
		$ad_no=$_GET["ad-no"];
	}
	if ($_SESSION["company_registration_number"] != $_GET["crn"]) {
		header("location:providerdashboard-ea.php?crn=wrong");
	}
 ...
 <?php  
 	...
 	$query ="SELECT * FROM job_ad WHERE ad_no='{$ad_no}' AND company_registration_number='{$company_registration_number}' AND is_delete=0 LIMIT 1";
 	...
 	if ($result) {
 		if (mysqli_num_rows($result)==1) {
 			...
 			$ad_result=mysqli_fetch_assoc($result);

 				$job_title=$ad_result["job_title"];
	 			$email=$ad_result["email"];
	 			$company_url=$ad_result["company_url"];

	 			$location =explode(",",$ad_result["location"]);
	 			//print_r($location);

	 			$job_type=$ad_result["job_type"];
	 			$phone_number=$ad_result["phone_number"];
	 			$monthly_salary=$ad_result["monthly_salary"];
	 			$job_category=$ad_result["job_category"];
	 			$gender=$ad_result["gender"];
 			...
 			$error[]="This Advertisement Can not Edit";
 ...
 <?php 
 		...
 		if (isset($_POST["submit"])) {
	 		...
	 		if (empty($error)) {
	 			...
	 			if ($result) {
	 				echo "<script>";
	 				echo "alert('Advertisement Updated')";
	 				
	 				echo "</script>";
	 				header('location:providerdashboard-ea.php?p=' . $p);
...
<body>
	...
	<div class="section">
		<div class="add-content">
			...
			<form action="providerdashboard-ad-edit.php?crn=<?php echo $company_registration_number; ?>&ad-no=<?php echo $ad_no ?>&p=<?php echo $p; ?>" method="post">
				<div class="row2">
					<p>
					...
					<p>
						...
						<!--<span class="sow_town" style="margin-left: 15px; font-size: 15px; color:#706c6c">
							<?php //echo $ad_result["location"] ?>
						</span>-->
						<select name="location[]" id="location" data-placeholder="<?php echo $ad_result["location"]; ?>" multiple>
	...
	<script>
                ...
                <script src="js/multi-chooser/chosen.jquery.min.js"></script>	
